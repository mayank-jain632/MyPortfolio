    ...
    >
      <div className="section-shell fade-in">
        ...
        <div className="grid gap-6 md:grid-cols-[minmax(0,2.2fr)_minmax(0,2.3fr)]">
            ...
            {experiences.map((exp, idx) => {
              ...
              return (
                <button
                  ...
                  className={`glass-panel w-full rounded-2xl px-4 py-3 text-left text-xs transition-all sm:px-5 sm:py-4 ${
                    active
                      ? "border-accent-500/80 shadow-glow"
                      : "hover:border-accent-500/50 hover:bg-slate-900"
                  }`}
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {exp.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full bg-slate-800/90 px-2 py-0.5 text-[0.65rem] text-slate-200"
                      >
                        {tag}
                      </span>
                    ))}
          ...
          <div className="glass-panel h-full rounded-2xl p-4 text-xs text-slate-200 sm:p-5">
                ...
                <ul className="mt-3 space-y-2.5">
                  {experiences[expandedIndex].bullets.map((bullet, i) => (
                    <li key={i} className="flex gap-2">
                      <span className="mt-1 h-1 w-4 rounded-full bg-gradient-to-r from-sky-400 to-cyan-400" />
                      <p>{bullet}</p>
                    </li>
                  ))}
                </ul>
